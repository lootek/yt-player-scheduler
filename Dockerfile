...
# Pin Node.js to 22.x from NodeSource and yt-dlp to a known-good nightly build.
# Newer yt-dlp nightlies have tightened YouTube rate-limiting and returned 403s
# for downloads that previously worked, so we intentionally do NOT use
# --update-to nightly or the latest release URL.
RUN apt-get update && apt-get install -y --no-install-recommends ca-certificates curl gnupg && \
    ...
    curl -fsSL https://github.com/yt-dlp/yt-dlp-nightly-builds/releases/download/2026.06.21.235142/yt-dlp -o /usr/local/bin/yt-dlp && \
